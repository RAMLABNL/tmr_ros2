...
public:
	...
	static std::vector<double> mmdeg_pose(const std::vector<double> &pose)
	{
	...
	static std::vector<double> m_rad_pose(const std::vector<double> &pose)
	{
		std::vector<double> rv(pose.size());
		if (pose.size() == 6) {
...
/*	Triggering IO high/low.
	...
	static std::string set_io(IOModule module, IOType type, int pin, float state);
...
/*	Setting target, velocity and time.
	...
	static std::string set_pvt_point(PvtMode mode,
		double t, const std::vector<double> &pos, const std::vector<double> &vel, int precision = 5);
	
/*	Setting target, velocity and time.
	PvtMode :	Joint, Tool
	PvtPoint:	time ,positions and velocities;
	More details please refer to the TM_Robot_Expression.pdf Chapter 9.18 */
	static std::string set_pvt_point(PvtMode mode, const PvtPoint &point, int precision = 5)
	{
		return set_pvt_point(mode, point.time, point.positions, point.velocities, precision);
	}

/*	Setting target, velocity and total time.
	PvtTraj 	: PvtMode mode
	points		: target
	total_time	: total time
	More details please refer to the TM_Robot_Expression.pdf Chapter 9.18 */
	static std::string set_pvt_traj(const PvtTraj &pvts, int precision = 5);
