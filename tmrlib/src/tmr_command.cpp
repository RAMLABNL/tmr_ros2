...
std::string Command::set_tag(int tag, int wait)
{
	std::stringstream ss;
	ss << "QueueTag(" << tag << "," << wait << ")";
	return ss.str();
}
std::string Command::set_wait_tag(int tag, int timeout_ms)
{
	std::stringstream ss;
...
std::string Command::set_joint_pos_PTP(const std::vector<double> &angs,
...
std::string Command::set_tool_pose_PTP(const std::vector<double> &pose,
	int vel_percent, double acc_time, int blend_percent, bool fine_goal, int precision)
{
...
std::string Command::set_tool_pose_Line(const std::vector<double> &pose,
...
std::string Command::set_pvt_point(PvtMode mode,
...
std::string Command::set_pvt_traj(const PvtTraj &pvts, int precision)
{
	std::stringstream ss;
	ss << std::fixed << std::setprecision(precision);
...
std::string Command::set_vel_mode_start(VelMode mode, double timeout_zero_vel, double timeout_stop)
{
...
std::string Command::set_vel_mode_target(VelMode mode, const std::vector<double> &vel, int precision)
{
	std::stringstream ss;
	ss << std::fixed << std::setprecision(precision);
	if (mode == VelMode::Joint) {
